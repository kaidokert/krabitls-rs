...
//! server pinned to secp256r1 presenting an ECDSA-P256 leaf). Drives a seed-0
//! P-256 ECDHE + AES-128-GCM krabitls client that advertises ECDSA signature
//! algorithms over a byte-recording transport and writes the
//! `testdata/packets_p256_ecdsa/` replay fixtures (an all-P-256 handshake:
//! P-256 KEX *and* an ECDSA-P256 server certificate).
//!
//! krabitls offers X25519 *and* secp256r1, so the server must be restricted to
//! P-256 to force the classical ECDHE path. Self-signed ECDSA prime256v1 leaf
//! whose SAN matches the client hostname, no tickets (deterministic flight):
//!
//! ```text
...
//! openssl s_server -accept 14464 -tls1_3 -groups P-256 -num_tickets 0 \
//!   -cert ecsrv.crt -key ecsrv.key -quiet -www
//! ```
//!
//! ```text
//! KB_PORT=14464 cargo test --no-default-features --features cipher-aes,ecdsa,p256-kx \
//!   --test gen_p256_ecdsa_fixtures -- --ignored --nocapture capture_p256_ecdsa_fixtures
//! ```
...
#[test]
#[ignore = "capture harness: needs a local P-256-only ECDSA TLS server (see module docs)"]
fn capture_p256_ecdsa_fixtures() {
    ...
    let tls = DefaultStream::connect(&params, &mut scratch, tee, &mut rng)
        .expect("seed-0 P-256 ECDHE + ECDSA handshake");
