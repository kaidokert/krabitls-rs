...
//! echo server). Drives a seed-0 X25519MLKEM768 + AES-128-GCM krabitls client
//! over a byte-recording transport and writes the `testdata/packets_mlkem/`
//! replay fixtures (`canned_handshake_mlkem.rs` replays them with no network).
//!
//! krabitls offers *only* X25519MLKEM768, so the server just has to support it
//! (openssl 3.5+ does) — no server-side group selection is needed. The server
//! encapsulates to the client's seed-0 ML-KEM ek, and the recorded ciphertext
//! is decapsulated by the same seed-0 client at replay, so the hybrid secret
...
//! srv = socket.create_server(("127.0.0.1", 14436))
//! while True:
//!     c, _ = srv.accept()
...
//!
//! ```text
//! KB_PORT=14436 cargo test --no-default-features --features cipher-aes,mlkem \
//!   --test gen_mlkem_fixtures -- --ignored --nocapture capture_mlkem_fixtures
//! ```
...
fn capture_mlkem_fixtures() {
    ...
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/../testdata/packets_mlkem");
    let write = |name: &str, desc: &str, bytes: &[u8]| {
        let body = format!(
            "# krabitls seed-0 X25519MLKEM768 + AES-128-GCM {desc} ({} bytes), captured\n\
             # from a local fixed-reply TLS echo server. Regenerate with the `#[ignore]`d\n\
             # `gen_mlkem_fixtures` test; do not hand-edit.\n\
             {}\n",
            bytes.len(),
            to_hex(bytes),
        );
