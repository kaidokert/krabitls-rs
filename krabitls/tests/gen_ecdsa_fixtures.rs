...
//! ECDSA signature algorithms against a server presenting a self-signed P-256
//! leaf, and writes the `testdata/packets_ecdsa/` replay fixtures
//! (`canned_handshake_ecdsa.rs` replays them with no network).
//!
//! ECDSA here is the server *certificate* signature (KEX stays plain X25519);
//! the client verifies the self-signed P-256 cert + the ECDSA-P256
//! CertificateVerify. Drop-in analogue of the RSA / ML-DSA fixtures.
//!
//! Server — a self-signed P-256 leaf whose SAN matches the client hostname,
//! plus a tiny fixed-reply TLS echo server (`num_tickets=0`):
//!
//! ```text
//! openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:P-256 \
//!   -keyout server.key -out server.crt -days 36500 -nodes \
...
//! srv = socket.create_server(("127.0.0.1", 14463))
//! while True:
//!     c, _ = srv.accept()
...
//!
//! ```text
//! KB_PORT=14463 cargo test --no-default-features --features cipher-aes,ecdsa \
//!   --test gen_ecdsa_fixtures -- --ignored --nocapture capture_ecdsa_fixtures
//! ```
...
fn capture_ecdsa_fixtures() {
    let port = std::env::var("KB_PORT").unwrap_or_else(|_| "14463".into());
    ...
    let dir = concat!(env!("CARGO_MANIFEST_DIR"), "/../testdata/packets_ecdsa");
    let write = |name: &str, desc: &str, bytes: &[u8]| {
        let body = format!(
            "# krabitls seed-0 AES-128-GCM + ECDSA-P256 server-cert {desc} ({} bytes),\n\
             # captured from a local fixed-reply TLS echo server. Regenerate with the\n\
             # `#[ignore]`d `capture_ecdsa_fixtures` test; do not hand-edit.\n\
             {}\n",
            bytes.len(),
            to_hex(bytes),
        );
