...
//! `testdata/packets_mtls_ecdsa/` replay fixtures for the footprint suite's
//! ECDSA-mTLS stack measurement.
//!
//! Server — ECDSA P-256 self-signed leaf (SAN=tls-fixture.local), mutual auth
//! against the client CA (`clientca.crt`) that signed `client_leaf.der`:
//!
//! ```text
//! openssl ecparam -genkey -name prime256v1 -noout -out ecsrv.key
//! openssl req -x509 -new -key ecsrv.key -out ecsrv.crt -days 2 \
//!   -subj "/CN=tls-fixture.local" -addext "subjectAltName=DNS:tls-fixture.local"
//! openssl s_server -accept 14438 -tls1_3 -cert ecsrv.crt -key ecsrv.key \
//!   -Verify 1 -CAfile clientca.crt -ciphersuites TLS_AES_128_GCM_SHA256 \
//!   -groups X25519 -www -quiet
//! ```
//!
//! ```text
//! KB_PORT=14438 cargo test --features ecdsa --test gen_mtls_ecdsa_fixtures \
//!   -- --ignored --nocapture capture_mtls_ecdsa_fixtures
//! ```
...
const CLIENT_LEAF_DER: &[u8] = include_bytes!("../../testdata/packets_mtls_ecdsa/client_leaf.der");
...
fn capture_mtls_ecdsa_fixtures() {
    let port = std::env::var("KB_PORT").unwrap_or_else(|_| "14438".into());
    ...
    let tls = DefaultStream::connect(&params, &mut scratch, tee, &mut rng)
        .expect("seed-0 ECDSA mTLS handshake");
    ...
    let dir = concat!(
        ...
        "/../testdata/packets_mtls_ecdsa"
    );
    let write = |name: &str, desc: &str, bytes: &[u8]| {
        let body = format!(
            "# krabitls seed-0 AES-128-GCM ECDSA-P256-mTLS {desc} ({} bytes),\n\
             # captured from a local openssl s_server -Verify handshake. Regenerate\n\
             # with the `#[ignore]`d `gen_mtls_ecdsa_fixtures` test; do not hand-edit.\n\
             {}\n",
            bytes.len(),
            to_hex(bytes),
        );
    ...
    eprintln!(
        "wrote ECDSA-mTLS fixtures: ch={} sh={} server_flight={} client_flight={}",
        tx[0].len(),
        rx[0].len(),
        flight.len(),
        tx[1].len()
