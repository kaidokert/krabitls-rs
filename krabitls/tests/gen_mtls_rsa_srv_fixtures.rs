...
//! `testdata/packets_mtls_rsa_srv/` replay fixtures for the footprint suite's
//! RSA-mTLS-with-RSA-server stack measurement.
//!
//! Unlike `gen_mtls_rsa_fixtures` (ed25519 server leaf), the server here has an
//! RSA-2048 leaf, so server-auth (RSA verify) and client-auth (RSA sign) run the
//! same algorithm. Mutual auth against a client CA that signed the fixture's RSA
//! leaf (`client_leaf.der`; private half in `common/rsa_client_key.rs`):
//!
//! ```text
//! openssl req -x509 -newkey rsa:2048 -keyout srv_rsa.key -out srv_rsa.crt -days 36500 -nodes \
...
//! openssl s_server -accept 14439 -tls1_3 -cert srv_rsa.crt -key srv_rsa.key \
//!   -Verify 1 -CAfile clientca.crt -ciphersuites TLS_AES_128_GCM_SHA256 -groups X25519 -www -quiet
//! ```
//!
//! ```text
//! KB_PORT=14439 cargo test --features rsa --test gen_mtls_rsa_srv_fixtures \
//!   -- --ignored --nocapture capture_mtls_rsa_srv_fixtures
//! ```
...
fn capture_mtls_rsa_srv_fixtures() {
    let port = std::env::var("KB_PORT").unwrap_or_else(|_| "14439".into());
    ...
    let dir = concat!(
        ...
        "/../testdata/packets_mtls_rsa_srv"
    );
    let write = |name: &str, desc: &str, bytes: &[u8]| {
        let body = format!(
            "# krabitls seed-0 AES-128-GCM RSA-server RSA-mTLS {desc} ({} bytes),\n\
             # captured from a local openssl s_server -Verify handshake. Regenerate\n\
             # with the `#[ignore]`d `gen_mtls_rsa_srv_fixtures` test; do not hand-edit.\n\
             {}\n",
            bytes.len(),
            to_hex(bytes),
        );
