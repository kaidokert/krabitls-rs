...
/// Verify a self-signed X.509 cert (Ed25519 or RSA-PKCS#1-v1.5-SHA-256)
/// by checking the cert's signature against the cert's own SubjectPublicKey.
/// Returns the parsed `CertView` for downstream `CertificateVerify` dispatch.
///
...
/// `E::Cache` (always) and `rsa_cache` (only under `feature = "rsa"`)
/// to amortize the per-key precomputes when the caller is going to
/// verify more signatures against the same cert (notably the
...
///
/// `E` is the Ed25519 verify backend; see [`Ed25519Verify`]. The RSA branch
/// (`feature = "rsa"`) uses `crate::backends::rsa_verify` directly and doesn't go
/// through `E`.
...
/// `E::Cache` (always) and `rsa_cache` (only under `feature = "rsa"`)
/// to amortize the per-key precomputes. Used by [`verify_server_flight`]
/// so a single cache is shared across the pipeline's two verifies.
pub fn verify_certificate_verify_with_cache<E: Ed25519Verify>(
    ...
    match (scheme, cert_view) {
        ...
        (
            ...
            // sig_len must equal RSA modulus byte length.
...
pub enum ServerPubkey<'a> {
    ...
    /// RSA pubkey: arbitrary-length big-endian modulus + small public
    /// exponent. Modulus borrowed from the cert DER, lifetime-bound to
    /// the `plaintext` that was passed to `verify_server_flight`.
    #[cfg(feature = "rsa")]
...
impl<'a> ServerPubkey<'a> {
    ...
    /// If the variant is Ed25519, return the 32-byte pubkey. With
    /// `feature = "rsa"` off the Ed25519 variant is the only inhabitant so
    /// the pattern is irrefutable — silence that warning rather than splitting
    /// the implementation across cfgs.
...
pub fn verify_server_flight<'a, H: HkdfSha256, C: CertParser, E: Ed25519Verify>(
    ...
    // For RSA certs: pre-build the cached PKCS#1 + PSS verifying keys
    // ONCE (one `ModMathParams::new` precompute, ~400-800k cycles for
    // U2048 on M3). Both self-sig and CertificateVerify below reuse it.
    // Ed25519 certs get `rsa_cache = None` and pay nothing here.
    #[cfg(feature = "rsa")]
