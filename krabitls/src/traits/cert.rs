...
pub enum CertView<'a> {
    ...
    Ed25519 {
        ...
        /// notBefore / notAfter dates are parsed on demand by the
        /// `verify_validity` check under `feature = "validity"`.
        validity_der: &'a [u8],
    },
...
impl<'a> CertView<'a> {
    ...
    /// compares against a caller-supplied [`crate::time::TimeSource`].
...
pub enum CertParseError {
    ...
    TrailingBytes,
    /// An `AlgorithmIdentifier` names an OID we don't recognize. The known
    /// set is Ed25519 (`1.3.101.112`) for both cert sig and SPKI;
    /// `sha256WithRSAEncryption` (`1.2.840.113549.1.1.11`) for the cert sig;
    /// `rsaEncryption` (`1.2.840.113549.1.1.1`) for the SPKI.
    WrongAlgorithmOid,
    ...
    /// only version this client accepts).
    UnsupportedCertVersion,
    /// SPKI's pubkey algorithm doesn't match the cert's signature algorithm
    /// family. e.g. a TBS signed `sha256WithRSAEncryption` but the SPKI
    /// claims Ed25519 — self-signed certs MUST agree on the key type.
