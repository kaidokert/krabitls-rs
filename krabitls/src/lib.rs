...
pub use backends::{DerCert, JedisctCrypto, RustCrypto};
...
pub mod consts {
    ...
    /// `rsa_pss_rsae_sha256` (RFC 8446 §4.2.3) — RSA-PSS w/ SHA-256 over an
    /// RSAE public key. Advertised when `feature = "rsa"` is enabled.
...
mod tests {
    ...
    /// (`packets/001_c2s_ClientHello.bin`). Fixed at 117 bytes regardless of
    /// feature config — it's the fixture's wire bytes, not what krabitls
    /// would emit. With `feature = "rsa"` enabled, krabitls's CH grows to
    /// 119 bytes (extra sig scheme); byte-identity tests are cfg-gated.
    ...
    fn hkdf_expand_label_rejects_oversized_public_inputs() {
        ...
        assert_eq!(
            hkdf_expand_label::<JedisctCrypto>(&secret, b"ok", b"", &mut huge_out),
            Err(HkdfLabelError::Expand(
                traits::HkdfExpandError::OutputTooLong
            ))
        );
    ...
    fn jedisct_matches_rustcrypto() {
        ...
        let rc_hs = handshake_secret::<RustCrypto>(&dhe);
        let jd_hs = handshake_secret::<JedisctCrypto>(&dhe);
        ...
        let rc_ts = handshake_traffic_secrets::<RustCrypto>(&rc_hs, &th);
        let jd_ts = handshake_traffic_secrets::<JedisctCrypto>(&jd_hs, &th);
    ...
    fn fixture_packet_3_decrypts() {
        ...
        assert_eq!(content_type, CT_HANDSHAKE);
    ...
    fn fixture_packet_3_decrypts_full_chain() {
        ...
        let (_c_ts, s_ts) = handshake_traffic_secrets::<RustCrypto>(&hs, &th);
    ...
    // ---- server_flight: parse + verify cert sig + CertVerify + Finished ----
    ...
    #[test]
    fn cert_rejects_wrong_outer_signature_algorithm_oid() {
        // Flip the last byte of the outer signatureAlgorithm OID. Outer-OID
        // check fires first in the parser, before symmetry / SPKI.
        let mut buf = [0u8; 512];
        let len = fixture_cert_der_copy(&mut buf);
    ...
    fn cert_rejects_wrong_spki_algorithm_oid() {
        ...
        // Surfaces as AlgorithmFamilyMismatch because the SPKI OID needs to
        // match the cert-sig kind (Ed25519 here) — a different but still-
        // recognized OID would fail with the same variant.
        let mut buf = [0u8; 512];
        let len = fixture_cert_der_copy(&mut buf);
        let positions = find_ed25519_oid_positions(&buf[..len]);
        buf[positions[1] + 4] ^= 0x01; // SPKI algorithm OID
        let err = <DerCert as CertParser>::parse(&buf[..len]).unwrap_err();
        assert_eq!(err, CertParseError::AlgorithmFamilyMismatch);
    }
    ...
    // ---- app data: encrypt/decrypt round trip against fixture packets 5 + 6 ----

    /// packets/005_c2s_AppData_send_0.bin (52 bytes) — first client app-data record.
    const FIXTURE_PACKET_5: &[u8] =
    ...
    fn application_keys() -> (ApAeadKeys, ApAeadKeys) {
        ...
        .unwrap();
        let ms = master_secret::<RustCrypto>(&FIXTURE_HANDSHAKE_SECRET);
        let (c_ap_ts, s_ap_ts) =
            application_traffic_secrets::<RustCrypto>(&ms, &transcript.snapshot());
        (
            traffic_keys::<RustCrypto>(&c_ap_ts),
            traffic_keys::<RustCrypto>(&s_ap_ts),
        )
    }
    ...
    fn fixture_application_traffic_secrets_match() {
        ...
        let (c_ap, s_ap) = application_traffic_secrets::<RustCrypto>(&ms, &transcript.snapshot());

        // Expected values from packets/003 dump notes (seed 0).
        const FIXTURE_C_AP: Secret = Secret::new([
        ...
        const FIXTURE_S_AP: Secret = Secret::new([
        ...
        assert_eq!(c_ap, FIXTURE_C_AP);
        assert_eq!(s_ap, FIXTURE_S_AP);
    }
    ...
    fn bad_tag_returns_aead_failed() {
        ...
        buf.fill(0xAA);
        let err = decrypt_record::<RustCrypto>(
            &tampered,
            &FIXTURE_S_HS_KEY,
            &FIXTURE_S_HS_IV,
            0,
    ...
    fn decrypt_record_rejects_trailing_bytes() {
        ...
        let mut buf = [0u8; 400];
        let err =
            decrypt_record::<RustCrypto>(&extra, &FIXTURE_S_HS_KEY, &FIXTURE_S_HS_IV, 0, &mut buf)
                .unwrap_err();
    ...
    // -------------------------------------------------------------------
    // RSA fixture tests (feature = "rsa")
    //
    // Cross-check against tls_fixture/packets_rsa/, which the Python fixture
    // produces with --auth rsa: a hardcoded RSA-2048 server identity signing
    // CertificateVerify with rsa_pss_rsae_sha256. Confirms the cert parser
    // + verify_server_flight pipeline accept RSA-signed server flights.
    // -------------------------------------------------------------------
    ...
    mod rsa_tests {
        ...
        /// packets_rsa/001_c2s_ClientHello.bin (117 bytes — same shape as
        /// ed25519 mode; the fixture's sig_algs ext is auth-gated).
        const FIXTURE_RSA_CLIENT_HELLO: &[u8] =
            include_bytes!("../../testdata/packets_rsa/001_c2s_ClientHello.bin");
        /// packets_rsa/002_s2c_ServerHello.bin
        const FIXTURE_RSA_SERVER_HELLO: &[u8] =
            include_bytes!("../../testdata/packets_rsa/002_s2c_ServerHello.bin");
        /// packets_rsa/003_s2c_ServerFlight_encrypted.bin (1034 bytes —
        /// dominated by the 2048-bit RSA cert + 256-byte RSA-PSS signature).
        const FIXTURE_RSA_PACKET_3: &[u8] =
            include_bytes!("../../testdata/packets_rsa/003_s2c_ServerFlight_encrypted.bin");

        /// Server handshake traffic secret from packets_rsa/004 notes.
        const FIXTURE_RSA_S_HS_TRAFFIC_SECRET: Secret = Secret::new([
        ...
        fn fixture_rsa_server_flight_verifies() {
            ...
            let (key, iv) = traffic_keys::<RustCrypto>(&FIXTURE_RSA_S_HS_TRAFFIC_SECRET);
            ...
            verify_server_flight::<RustCrypto, DerCert, RustCrypto>(
                &mut transcript,
                content,
                &FIXTURE_RSA_S_HS_TRAFFIC_SECRET,
            )
