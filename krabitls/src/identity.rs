...
/// [`crate::ServerPubkey`] — the lifetime ties to a borrowed RSA modulus
/// when the RSA feature is on.
...
pub enum PinnedPubkey<'a> {
    ...
    /// Lifetime binding for the Ed25519 variant when `feature = "rsa"` is off.
    #[doc(hidden)]
    _Phantom(core::marker::PhantomData<&'a ()>),
}

/// Compare the cert's public key to a pinned reference. Constant-time over
/// the key bytes is **not** attempted — see #16 in PRODUCTION_GAPS for why
/// `subtle` isn't pulled in here.
...
// =====================================================================
// Cert validity-window check (PRODUCTION_GAPS #5).
...
/// supplied [`crate::traits::time::TimeSource`].
///
...
fn parse_validity_der(der: &[u8]) -> Result<(u64, u64), ValidityError> {
    // Outer SEQUENCE header: tag 0x30, length.
...
/// rest_after_header)`. Same shape as the SAN walker's
/// `decode_tlv_header` above but reports through `ValidityError` so
/// `?` flows cleanly inside the validity path.
#[cfg(feature = "validity")]
...
/// or pre-1970 (cert validity can't predate the epoch in any
/// realistic deployment).
///
/// Algorithm: count days since 1970-01-01 using the standard
/// civil-from-days reverse trick (Howard Hinnant's date algorithms,
/// adapted for forward conversion). Avoids leap-year edge cases by
/// shifting Feb to the "end of the year."
#[cfg(feature = "validity")]
fn days_to_epoch_secs(y: u32, m: u32, d: u32, h: u32, mn: u32, s: u32) -> Option<u64> {
    if y < 1970 || m == 0 || m > 12 || d == 0 || d > 31 || h > 23 || mn > 59 || s > 60 {
        return None;
    }
    // Hinnant civil-to-days. y' = year shifted so March is month 1 of
    // year y' (Jan/Feb count as months 11/12 of year y'-1). era = era
    // of 400 years.
...
mod tests {
    ...
    mod validity_tests {
        ...
        /// 2020-01-15T00:00:00Z = 1579046400 epoch seconds (cross-checked vs `date -u`).
        const T_2020_01_15: u64 = 1579046400;
        /// 2030-01-15T00:00:00Z = 1894665600 (= T_2020_01_15 + 10y + 3 leap days).
        ...
        fn days_to_epoch_rejects_pre_epoch_and_bad_dates() {
            ...
            assert!(days_to_epoch_secs(2024, 0, 1, 0, 0, 0).is_none()); // month=0
            assert!(days_to_epoch_secs(2024, 13, 1, 0, 0, 0).is_none()); // month=13
            assert!(days_to_epoch_secs(2024, 1, 0, 0, 0, 0).is_none()); // day=0
            assert!(days_to_epoch_secs(2024, 1, 32, 0, 0, 0).is_none()); // day=32
        }

        #[test]
        fn decode_utctime() {
            // 200115000000Z = UTCTime "2020-01-15T00:00:00Z"
        ...
        fn decode_utctime_pivot_at_50() {
            ...
            // rejects pre-1970, so test the YY=99 → 1999 boundary which
            // is the latest pre-1970-safe value, vs YY=49 → 2049.
            let yy_49 = decode_time(0x17, b"490101000000Z").unwrap();
            let yy_99 = decode_time(0x17, b"991231235959Z").unwrap();
            // 2049-01-01 should be later than 1999-12-31.
            assert!(yy_49 > yy_99);
            // YY=50 → 1950, which is pre-1970 and therefore rejected.
            assert!(decode_time(0x17, b"500101000000Z").is_err());
        }

        #[test]
        fn decode_generalizedtime() {
            // 20300115000000Z = GeneralizedTime "2030-01-15T00:00:00Z"
        ...
        fn decode_time_rejects_bad_shapes() {
            // Missing trailing Z
            assert!(decode_time(0x17, b"200115000000X").is_err());
            // Wrong length for tag
            assert!(decode_time(0x17, b"2001150000Z").is_err());
            // Non-digit
            assert!(decode_time(0x17, b"20A115000000Z").is_err());
            // Wrong tag
            assert!(decode_time(0x16, b"200115000000Z").is_err());
        }
        ...
        fn validity_ok_in_window() {
            ...
            // T_2024_06_15_LATE is well inside [2020-01-15, 2030-01-15).
            assert_eq!(
        ...
        fn validity_not_yet_valid() {
            // Cert valid 2030+; "now" is 2024 → NotYetValid.
        ...
        #[test]
        fn validity_expired() {
            // Cert valid 2010..2020; "now" is 2024 → Expired.
        ...
        fn validity_malformed_der_rejected() {
            // Garbage bytes — not a SEQUENCE at all.
        ...
        fn fixed_time_is_a_timesource() {
            // Sanity: the FixedTime helper actually implements TimeSource.
