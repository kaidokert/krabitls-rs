...
fn write_client_hello_with_aes_only_narrows_cipher_suites() {
    let x25519 = || {
        crate::backends::ecdhe_x25519::EcdheX25519::from_secret_bytes(&FIXTURE_CLIENT_X25519_PRIV)
            .unwrap()
    };
    let conn_default: TlsConnection<Init, RustCrypto> =
        TlsConnection::new(FIXTURE_RANDOM, x25519());
    ...
    let conn_aes: TlsConnection<Init, RustCrypto> = TlsConnection::new(FIXTURE_RANDOM, x25519());
...
mod aes_only {
    ...
    fn fixture_prepared() -> PreparedVerifier<RustCrypto> {
        PreparedVerifier::ed25519(
            <RustCrypto as crate::traits::verify_provider::SigVerifierProvider<
                crate::traits::verify_provider::Ed25519,
            >>::prepare(&EXPECTED_SERVER_ID_PUB)
    ...
    fn feed_server_record_and_finalize_smoke() {
        ...
        let done = conn
            .finalize_server_flight::<512, RustCrypto>(
                &reassembler,
                &fixture_prepared(),
                &fixture_leaf_view(),
            )
    ...
    fn finalize_without_flight_is_incomplete() {
        ...
        let err = match conn.finalize_server_flight::<512, RustCrypto>(
            &reassembler,
            &fixture_prepared(),
            &fixture_leaf_view(),
        ) {
