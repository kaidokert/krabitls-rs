...
use krabitls::{
    CLIENT_FINISHED_LEN, DerCert, JedisctCrypto, RustCrypto, TranscriptHash,
...
fn main() -> ! {
    cortex_m_demo::test_fixture(
        || {
            ...
            let hs = handshake_secret::<JedisctCrypto>(&dhe);
            ...
            let (_c_ts, s_ts) =
                handshake_traffic_secrets::<JedisctCrypto>(&hs, &transcript.snapshot());
            let (key, iv) = traffic_keys::<JedisctCrypto>(&s_ts);
            ...
            let verified = match verify_server_flight::<JedisctCrypto, DerCert, RustCrypto>(
                &mut transcript,
                content,
                &s_ts,
            ) {
            ...
            let mut out = [0u8; 64];
            let record = match build_client_finished::<JedisctCrypto, RustCrypto>(
            ...
            let ms = master_secret::<JedisctCrypto>(&hs);
            let (c_ap_ts, s_ap_ts) =
                application_traffic_secrets::<JedisctCrypto>(&ms, &th_through_sf);
            let (c_ap_key, c_ap_iv) = traffic_keys::<JedisctCrypto>(&c_ap_ts);
            let (s_ap_key, s_ap_iv) = traffic_keys::<JedisctCrypto>(&s_ap_ts);
