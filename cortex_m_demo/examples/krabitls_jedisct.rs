...
//! Same replay as `examples/krabitls.rs` (AES-128-GCM + Ed25519) but with
//! the HKDF / SHA-256 backend swapped to jedisct1's `hmac-sha256` crate.
//! Used to compare alternate-backend footprint against the RustCrypto baseline.
...
#[cfg(not(feature = "baseline"))]
use krabitls::{
    CLIENT_FINISHED_LEN, CertParser, CertView, DerCert, JedisctCrypto, RustCrypto, TranscriptHash,
    ZeroBuf, build_client_finished, decrypt_record, extract_cert_der, parse_server_flight,
    split_inner_plaintext, traffic_keys, verify_certificate_verify, verify_server_finished,
};

const CH: [u8; 137] = krabitls::hex_decode(include_str!("../captured/aes_ed25519/ch.hex"));
const SH: [u8; 95] = krabitls::hex_decode(include_str!("../captured/aes_ed25519/sh.hex"));
const S_HS_TS: [u8; 32] = krabitls::hex_decode(include_str!("../captured/aes_ed25519/s_hs_ts.hex"));
const C_HS_TS: [u8; 32] = krabitls::hex_decode(include_str!("../captured/aes_ed25519/c_hs_ts.hex"));
